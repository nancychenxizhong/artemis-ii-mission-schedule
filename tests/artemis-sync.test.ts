...
test("buildScheduleWithDeps applies live page updates into the bundled schedule", async () => {
  const pages = new Map<string, string>([
    ...
    [
      ...
      makeHtml(`Published: April 4, 2026 11:30 AM`),
    ],
    [
  ...
  assert.equal(schedule.syncMessage, "Live NASA sync succeeded for 7/7 sources.");
  assert.equal(schedule.sources.length, 7);
